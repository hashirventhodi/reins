---
description: Run the contract pipeline for a task, stopping only at human gates, escalations, or completion. Usage - /work <task-id>
---
...
1. Run `python3 ~/.claude/pipeline/core/pipeline_cli.py status $ARGUMENTS --json` and dispatch on status:
   - Before the first contract, if the log holds no disposition: run /dispose.
   ...
   - NEW/REFINING/RESEARCHING/PLANNING/IMPLEMENTING/REVIEWING/STALE/RETURNED:
     ...
     /review), then `python3 ~/.claude/pipeline/core/pipeline_cli.py validate $ARGUMENTS`; on violations,
   ...
   - UNVERIFIED: run /verify, then continue. Mechanical, not a human stop.
   ...
   - AWAITING_MERGE: STOP. Diff currency (D15): compare the review's diff pin
     ...
     review; warn and offer /review. Then `python3 ~/.claude/pipeline/core/pipeline_cli.py floor-check $ARGUMENTS` over
     the SOURCE paths (filtered as in /dispose): a lane violation must be
     resolved — escalate the lane or record an override — before merge. Then
     report and offer /followups.
