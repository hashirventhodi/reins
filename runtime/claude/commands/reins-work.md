---
description: Run the contract pipeline for a task, stopping only at human gates, escalations, or completion. Usage - /reins-work <task-id>
---
Task: $ARGUMENTS. Loop (normative; blueprint §6):
1. Run `python3 ~/.claude/reins/core/reins_cli.py status $ARGUMENTS --json` and dispatch on status:
   - Before the first contract, if the log holds no disposition: run /reins-dispose.
   ...
   - NEW/REFINING/RESEARCHING/PLANNING/IMPLEMENTING/REVIEWING/STALE/RETURNED:
     ...
     /reins-review), then `python3 ~/.claude/reins/core/reins_cli.py validate $ARGUMENTS`; on violations,
   ...
   - AWAITING_INTENT_CONSENT / AWAITING_PLAN_APPROVAL: STOP. Record the current
     hash (`python3 ~/.claude/reins/core/reins_cli.py hash <artifact>`), show the artifact, ask for approval or
     edits. Only on explicit human yes: re-hash; if changed pass --edited:
     `python3 ~/.claude/reins/core/reins_cli.py consent intent|plan $ARGUMENTS [--edited]`, then continue the loop.
   ...
   - UNVERIFIED: run /reins-verify, then continue. Mechanical, not a human stop.
   - BLOCKED: STOP. Present the escalation; on the human's routing run
     `python3 ~/.claude/reins/core/reins_cli.py decide returned $ARGUMENTS --to <contract> --reason "..."`, continue.
   - AWAITING_MERGE: STOP. Diff currency (D15): compare the review's diff pin
     ...
     review; warn and offer /reins-review. Then `python3 ~/.claude/reins/core/reins_cli.py floor-check $ARGUMENTS` over
     the SOURCE paths (filtered as in /reins-dispose): a lane violation must be
     resolved — escalate the lane or record an override — before merge. Then
     report and offer /reins-followups.
   - BYPASSED / DONE / INVALID: STOP and report; for DONE offer
     `/reins-followups $ARGUMENTS`.
