---
description: Choose the lane for a task - compute the floor, propose, record the human's decision. Usage - /reins-dispose <task-id>
---
...
2. `printf '%s\n' <paths> | python3 ~/.claude/reins/core/reins_cli.py floor $ARGUMENTS --lines-changed <n>`
...
4. Only on an explicit human reply:
   `python3 ~/.claude/reins/core/reins_cli.py decide disposition $ARGUMENTS --lane <lane> --floor <floor>`
