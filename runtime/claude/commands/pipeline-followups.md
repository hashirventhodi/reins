...
Run `python3 ~/.claude/pipeline/core/pipeline_cli.py followups $ARGUMENTS --json` — never derive candidates
...
JSON MECHANICALLY (jq or python -c, never retyped) and run:
  python3 ~/.claude/pipeline/core/pipeline_cli.py task add --title "<candidate.title>" --body-file <tmp file
